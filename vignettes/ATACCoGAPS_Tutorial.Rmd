...
BiocManager::install(c("CoGAPS", "GenomicRanges", "projectR", "TFBSTools", "GeneOverlap", "msigdbr", "motifmatchr", "chromVAR", "GenomicFeatures", "IRanges", "fgsea", "rGREAT", "Homo.sapiens", "Mus.musculus"))
...
To make use of this differential accessibility data, one option is to try to find genes that fall within these peaks and determine whether the accessibility of certain groups of genes suggests differential pathway activation. We choose peaks based on a threshold for the PatternMarker score of the peaks. Results are quite robust to changing the scoreThreshold, though we recommend generally using thresholds that return at least 100 peaks per pattern.
```{r, message = FALSE}
data("schepGranges")
...
We can alternatively perform enrichment for GO terms using the rGREAT package, which we employ via a wrapper function.
```{r}
GOterms <- findGOEnrichment(schepCogapsResult, schepGranges, "hg19", scoreThreshold = 0.05)

#top Embryonic Stem Cell pattern GO terms
GOterms[[5]][1:5,]
```
